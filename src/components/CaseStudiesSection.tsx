    ...
    <section className="py-24 relative overflow-hidden" style={{ backgroundColor: '#f5f3ef' }}>
      <div className="container mx-auto px-6 lg:px-12">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
          ...
          <h2 className="font-display text-4xl lg:text-5xl font-bold text-gray-900">{t.caseStudies.title} <GradientText>{t.caseStudies.titleHighlight}</GradientText></h2>
        </motion.div>
                  ...
                  <div className="h-full flex flex-col justify-between">
                    <div>
                      <div className="flex items-center justify-between mb-5">
                        <h3 className="font-display text-xl font-bold text-gray-900">{study.name}</h3>
                      ...
                      <div className="space-y-3">
                        <div className="flex justify-between border-b border-gray-200 pb-3">
                          ...
                          <span className="text-gray-900 text-sm text-right max-w-[65%]">{study.client}</span>
                        </div>
                        <div className="flex justify-between border-b border-gray-200 pb-3">
                          <span className="text-gray-500 text-xs uppercase tracking-wider">{t.caseStudies.goal}</span>
                          <span className="text-gray-900 text-sm text-right max-w-[65%]">{study.goal}</span>
                        </div>
                        <div className="flex justify-between border-b border-gray-200 pb-3">
                          <span className="text-gray-500 text-xs uppercase tracking-wider">{t.caseStudies.duration}</span>
                          <span className="text-gray-900 text-sm">{study.duration}</span>
                        </div>
                        <div className="pt-1">
                          <span className="text-gray-500 text-xs uppercase tracking-wider block mb-2">{t.caseStudies.result}</span>
                          <span className="text-[#94c43d] text-sm font-medium leading-relaxed block">{study.result}</span>
                        </div>
                      </div>
                    </div>
                    <Link to="/kontakt" className="mt-4 w-full inline-flex items-center justify-center gap-2 px-4 py-3 bg-[#e8e5e0] hover:bg-lime hover:text-[#1a1a1a] text-gray-900 rounded-full text-sm font-medium transition-all duration-300 group">
